...
`log(x, b)` is function of computing logarithms of `x` to the base `b`. And `log2(x)` is to compute logarithms with base 2.
...
We can create customized sequences using `seq()` function. Important arguments of the function are `from`, `to` and `by`. `from` defines start number, and `to` defines end values of the sequence. `by` defines increment of sequence.
...
It not consider order of the pair gene. R has function `choose(n,k)` to compute n choose x. 
...
A, B, C and C, B, A are different for counting the arrangement of 100 genes into triples. Number of the arrangement of 100 genes is 
$$
